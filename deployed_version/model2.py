...
class poem_classifier_model():
    ...
    def _save(self, emb, lr, optim):
        ...
        self.model.save_weights(f"./{emb}_{lr:.2f}_{optim}_acc_{max(self.trained_model.history['val_acc'])}.h5")     
    ...
    def classify_poem(self, verse):
        verse = self.tokenizer.texts_to_sequences([verse])
        padded_verse = tf.keras.utils.pad_sequences(verse, padding='pre',
                                           maxlen=self.max_len)
        _preds = np.sum(self.model.predict(padded_verse), axis=0)
        labels = ['Affection', 'Death', 'Environment', 'Music']
        return labels[np.argmax(_preds)]
